...
<div class="mt-6 bg-white shadow-sm rounded-lg divide-y">
    @foreach ($stores as $store)
        <div class="p-6 flex space-x-2" wire:key="{{ $store->id }}">
            ...
            <div class="flex-1">
                <div class="flex justify-between items-center">
                    <div>
                        <span class="text-gray-800">{{ $store->user->name }}</span>
                        <small class="ml-2 text-sm text-gray-600">{{ $store->created_at->format('j M Y, g:i a') }}</small>
                    </div>
                </div>
